// Main glossary: TOC links to category pages
...
function termCard(t,q){
  ...
  return `<div class="card term">
    ...
    <div class="meta">${t.category?`<span class="tag">${escapeHTML(t.category)}</span>`:''}${alias}</div>
...
function groupByCategory(items){
  ...
  return [...map.entries()].sort((a,b)=> a[0].localeCompare(b[0],'ja')).map(([cat,arr])=>[cat, arr.sort((x,y)=>(x.title||'').localeCompare(y.title||'', 'ja'))]);
}
function renderTOC(categories){
  ...
  box.innerHTML = categories.map(([cat, arr])=>{
    const href = `category-${slugify(cat)}.html`;
    return `<a href="${href}" class="toc-link">${escapeHTML(cat)} <span class="count">(${arr.length})</span></a>`;
...
async function render(q=''){
  ...
  root.innerHTML = groups.map(([cat,arr])=>{
    ...
    return `<section class="term-category"><h2 class="cat-heading">${escapeHTML(cat)}</h2><div class="cat-group">${cards}</div></section>`;
